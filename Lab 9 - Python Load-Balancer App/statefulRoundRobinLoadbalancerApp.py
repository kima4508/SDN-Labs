    ...
    def _packet_in_handler(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_IP and pkt.get_protocol(ipv4.ipv4).dst == self.loadBalancerIP:
            if pkt.get_protocol(tcp.tcp).dst_port == 8080:
                ...
                serverIP = None
                serverMAC = None
                if sourceIP in self.stateTable:
                    #send the request to the server   
                    serverIP = self.stateTable[sourceIP]
                    serverMAC = self.arp_table[serverIP] 
                    print(" * LoadBalancer state found in state table")
                else:
                    ...
                    serverMAC = self.arp_table[serverIP]
                    self.stateTable[sourceIP]=serverIP
                    print(" * Mapping state: "+sourceIP+" --> "+serverIP)
